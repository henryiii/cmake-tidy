...
fn main() {
    ...
    for file in &files {
        ...
        writeln!(
            out,
            "    let result = check_source(&source, &CheckOptions {{ project_root: false, function_name_case: NameCase::Lower }});"
        )
        .unwrap();
        writeln!(
            out,
            "    assert!(result.diagnostics.iter().all(|d| d.code != RuleCode::E001), \"fixture produced a parse error for {{}}: {{:#?}}\", path.display(), result.diagnostics);"
        )
        .unwrap();
        writeln!(out, "}}").unwrap();
        writeln!(out).unwrap();
